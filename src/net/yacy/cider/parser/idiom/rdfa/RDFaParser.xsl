...
<!-- named templates to generate RDF - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
  ...
  <template name="relation" >
    ...
    <choose>
     <when test="string-length($predicate-ns)>0"> <!-- there is a known namespace for the predicate -->
	      <choose>
	      	<when test="starts-with($subject,'blank:node:')"><value-of select="java:fr.inria.rdfa.RDFaParserImp.setTheSubjectNodeID($parser,substring-after($subject,'blank:node:'))"/></when>
	      	<otherwise><value-of select="java:fr.inria.rdfa.RDFaParserImp.setTheSubjectURI($parser,$subject)"/></otherwise>
	      </choose>
		  <!-- get full predicate -->
		  <variable name="expanded-predicate"><call-template name="expand-ns"><with-param name="qname" select="$single-predicate"/></call-template></variable>
          <value-of select="java:fr.inria.rdfa.RDFaParserImp.setThePropertyURI($parser,$expanded-predicate)"/>

	     <choose>
	      	<when test="starts-with($object,'blank:node:')"><value-of select="java:fr.inria.rdfa.RDFaParserImp.setTheObjectNodeID($parser,substring-after($object,'blank:node:'))"/></when>
	      	<otherwise><value-of select="java:fr.inria.rdfa.RDFaParserImp.setTheObjectURI($parser,$object)"/></otherwise>
	      </choose>
     </when>
    ...
    <value-of select="java:fr.inria.rdfa.RDFaParserImp.flushObjectProperty($parser)"/>
  ...
  <template name="property" >
    ...
    <choose>
     <when test="string-length($predicate-ns)>0"> <!-- there is a known namespace for the predicate -->
          ...
          <value-of select="java:fr.inria.rdfa.RDFaParserImp.setThePropertyURI($parser,$predicate-name)"/>
          
	      <choose>
	      	<when test="starts-with($subject,'blank:nod: ')"><value-of select="java:fr.inria.rdfa.RDFaParserImp.setTheSubjectNodeID($parser,substring-after($subject,'blank:node:'))"/></when>
	      	<otherwise><value-of select="java:fr.inria.rdfa.RDFaParserImp.setTheSubjectURI($parser,$subject)"/></otherwise>
	      </choose>
	            
	      <if test="string-length($language)>0"><value-of select="java:fr.inria.rdfa.RDFaParserImp.setTheLanguage($parser,$language)"/></if>

	      <choose>
	        <when test="$datatype='http://www.w3.org/1999/02/22-rdf-syntax-ns#XMLLiteral'">
	         <choose>
	         	<when test="$attrib='true'"> <!-- content is in an attribute -->
	         	  <value-of select="java:fr.inria.rdfa.RDFaParserImp.setTheDatatype($parser,$datatype)"/>
	         	  <value-of select="java:fr.inria.rdfa.RDFaParserImp.setTheValue($parser,normalize-space(string($object)))"/>
	            </when>
	         	<otherwise> <!-- content is in the element and may include some tags -->
	         	 <value-of select="java:fr.inria.rdfa.RDFaParserImp.setTheDatatype($parser,$datatype)"/>
				 <value-of select="java:fr.inria.rdfa.RDFaParserImp.setTheValue($parser,$object)"/>
				</otherwise>
			 </choose>
	        </when>
    ...
    <value-of select="java:fr.inria.rdfa.RDFaParserImp.flushDataProperty($parser)"/>
  ...
  <template name="class" >
    ...
    <if test="string-length($class-ns)>0"> <!-- we have a qname for the class -->
	       ...
	       <choose>
	      	<when test="starts-with($resource,'blank:node:')"><value-of select="java:fr.inria.rdfa.RDFaParserImp.setTheSubjectNodeID($parser,substring-after($resource,'blank:node:'))"/></when>
	      	<otherwise><value-of select="java:fr.inria.rdfa.RDFaParserImp.setTheSubjectURI($parser,$resource)"/></otherwise>
	       </choose>
   	     <value-of select="java:fr.inria.rdfa.RDFaParserImp.setThePropertyURI($parser,'http://www.w3.org/1999/02/22-rdf-syntax-ns#type')"/>
   	     <value-of select="java:fr.inria.rdfa.RDFaParserImp.setTheObjectURI($parser,$expended-class)"/>	 
		 <value-of select="java:fr.inria.rdfa.RDFaParserImp.flushObjectProperty($parser)"/>
	 </if>     
