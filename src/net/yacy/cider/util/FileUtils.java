...
public final class FileUtils {
    ...
    public static void copy(final InputStream source, final File dest, final long count) throws IOException {
        ...
        try {
            ...
            if (fos != null) try {fos.close();} catch (final Exception e) { Log.logWarning(log, "cannot close FileOutputStream for "+ dest +"! "+ e.getMessage()); }
        }
    }
    ...
    public static Map<String, String> loadMap(final File f) {
        ...
        try {
            ...
            Log.logSevere(log, f.toString() + " not found", e2);
            return null;
        }
    }
    ...
    public static void moveAll(final File from_dir, final File to_dir) {
        ...
        for (int i = 0; i < list.length; i++) {
            if(!new File(from_dir, list[i]).renameTo(new File(to_dir, list[i])))
                Log.logWarning(log, "moveAll(): could not move from "+ from_dir + list[i] +" to "+ to_dir + list[i]);
        }
    }
        ...
        if (path.exists()) {
            ...
            if (System.getProperties().getProperty("os.name","").toLowerCase().startsWith("windows")) {
                ...
                try {
                    ...
                    if (r == null) {
                        Log.logSevere(log, "cannot execute command: " + command);
                    } else {
                        byte[] response = read(r.getInputStream());
                        Log.logInfo(log, "deletedelete: " + new String(response));
                    }
                } catch (IOException e) {
                    Log.logException(e);
                }                
            }
            if (path.exists()) Log.logSevere(log, "cannot delete file " + p);
        }
    }
}
